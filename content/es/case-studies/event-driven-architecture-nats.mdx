...
sequenceDiagram
  ...
  participant BFF as social-bff (consumer durable)

  API->>API: termina el partido
  API->>NATS: publica uft.match.finished (fire-and-forget)
  Note over API: ¿NATS no alcanzable? se registra en logs y se ignora, nunca bloquea la respuesta
  NATS-->>BFF: entrega (at-least-once)
  BFF->>BFF: crea post de feed
  BFF-->>NATS: ack
```
