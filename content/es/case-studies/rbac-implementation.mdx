...
```mermaid
flowchart TD
  Req[Request entrante] --> Decl{"¿Declara @Roles o @RequirePermissions?"}
  Decl -->|Ninguno| Allow[Permitir — cualquier usuario autenticado]
  Decl -->|Alguno| Super{¿El rol es Super Administrador?}
  Super -->|Sí| Allow
  Super -->|No| AliasLookup[Resolver rol sembrado vía tabla de alias]
  AliasLookup --> Satisfies{¿Satisface el rol o permiso requerido?}
  Satisfies -->|Sí| Allow
  Satisfies -->|No| Deny[Denegar]
```
